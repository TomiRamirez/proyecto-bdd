...
El proceso en carga.sql utiliza tablas para transformar los datos de un CSV en un modelo relacional normalizado, asegurando la integridad referencial. 
...
a) Agenda Semanal (agenda.sql)
intente hacerla de mil maneras, pero en las fechas indicadas  (2026-04-06 - 2026-04-13) siempre se me generaba la tabla vacia, por lo cual no pude hacer esta consulta


b) Ingreso Mensual (ingresomensual.sql)

Se utiliza una serie de UNION ALL para clasificar y sumar los montos del mes actual en la sucursal "Santa Cruz". Se divide  en dos grandes categorías ("Ingresos Recibidos" e "Ingresos Esperados"), cruzando la información de pagos (cuotas, reservas, eventos) con los registros que aún están en estado pendiente (reservas no ejecutadas o cuotas sin pago registrado).

c) Socios Morosos (morosos.sql)

Se filtran las cuotas cuyo estado es estrictamente 'atrasado'. Mediante la función de agregación COUNT se calcula el número de cuotas adeudadas y con SUM el monto total, agrupando los resultados por el RUN y nombre del socio, además de asociar su sucursal base.


d) Fin de Beneficiario (finbeneficiario.sql)

Para calcular quiénes cumplen 29 años, se utiliza la función AGE() comparando la fecha de término de la membresía del titular con la fecha de nacimiento del beneficiario. como la fecha de nacimiento en algunos registros puede ser nula, se utiliza una subconsulta a la tabla de historial nacimiento para obtener la fecha de nacimiento del beneficiario.

e) Ingreso por Sucursal (ingresoporsucursal.sql)

Se estructuró la consulta utilizando Expresiones de Tabla Comunes (CTEs con la cláusula WITH) para lograr un código limpio. Primero se consolidan todos los ingresos del 2025 (cuotas, reservas y eventos) por sucursal. Luego se calcula el ingreso total del club y, mediante un CROSS JOIN, se calcula el porcentaje de participación de cada sucursal. Finalmente, se cruza con persona_cargo para incluir el nombre del gerente vigente.


## 2.4 archivos subidos

### carpetas: 
en cada una de estas carpetas esta el respectivo .csv con el OK.CSV y el .LOG.CSV
...
### sql: 
estan subidos los siguientes archivoos

carga.sql: con sus respectivas ERR.csv y cargaLOG.txt

ingresomensual.sql: con su ingresomensual.txt

morosos.sql: con su morosos.txt

finbeneficiario.sql: con su finbeneficiario.txt

ingresoporsucursal.sql: con su ingresoporsucursal.txt
...
dentro del servidor hay que correr el main

---
php main.php
---

y los archivos *OK.CSV y *LOG.CSV son generados por el main.php



se corre el archivo carga.sql

psql -d tgramirez.e2 -f carga.sql


Consultas sql: 

### a) agenda.sql
no pude hacer esta consulta :(


### b) ingresomensual.sql:

se corre el comando
---
psql -d tgramirez.e2 -f ingresomensual.sql
---

para ver el resultado de la consulta se corre el comando aunque igual se genera un txt

---
cat ingresomensual.txt
---
...
se corre el comando
---
psql -d tgramirez.e2 -f morosos.sql
---
...
### d) finbeneficiario.sql

se corre el comando

---
psql -d tgramirez.e2 -f finbeneficiario.sql
---

para ver el resultado de la consulta se corre el comando aunque igual se genera un txt

---
cat finbeneficiario.txt
---

### e) ingresoporsucursal.sql

se corre el comando
---
psql -d tgramirez.e2 -f ingresoporsucursal.sql
---

para ver el resultado de la consulta se corre el comando aunque igual se genera un txt

---
cat ingresoporsucursal.txt
---
...
me base en los materiales dubidos a github de este semestre

y ya he cursado anteriormente este ramo :)
